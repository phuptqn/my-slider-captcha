	...
	$.fn.sliderCaptcha = function( options ) {
		...
		return this.each( function() {
			...
			$this.find( '.swipe-knob' ).draggable({
				...
				cursor: 'move',
				revert: function() {
					var knob_width = $(this).outerWidth();
					var parent_width = $(this).parent('.slider_captcha').outerWidth();
					
					if (Math.round($(this).position().left) < Math.round(parent_width - knob_width)) {
						return true;
					}
			...
			$this.find( '.knob-destiny' ).droppable({
				...
				drop: function(event, ui) {
					drag_finished($(this), event, ui, s);
				}
			})
		});
	};

	var mouseOutOfSlider = false, gEl, gEvent, gUi, gS;
	$(document).mouseup(function() {
		if (mouseOutOfSlider)
			drag_finished(gEl, gEvent, gUi, gS);
	});

	var drag_finished = function($el, event, ui, s) {

		var events = $el.parent().data( 'events' ),
			$slider_elem = $el.parent(),
		...
		if ( events['afterUnlock'] &&  "function" == typeof( events['afterUnlock'] ) ) {
			...
			$el.closest('.wpcf7-form-control-wrap').find('.wpcf7-not-valid-tip').fadeOut();
		}
