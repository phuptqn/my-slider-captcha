...
class SliderCaptcha {
	...
	public function register_scripts() {
		...
		wp_enqueue_script('jquery-ui-touch-punch', plugins_url( '/js/jquery.ui.touch-punch-improved.js', __FILE__ ), array('jquery'), $this->jquery_plugin_verson, true);
	
		wp_enqueue_script('jquery-slider-captcha', plugins_url( '/js/slider-captcha.min.js', __FILE__ ), array('jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-ui-droppable', 'jquery-ui-draggable', 'jquery-ui-touch-punch'), $this->plugin_version, true);

		wp_enqueue_script('json2');
		
		wp_enqueue_style('slider-captcha-css', plugins_url( '/css/slider-captcha.css', __FILE__ ), false, $this->jquery_plugin_verson );
	...
	public function validate_register_slider($login, $email, $errors) {
		...
		if ( !$this->valid_request() ) {
			$errors->add( 'slider_captcha_missing', __("ERROR: Something went wrong with the CAPTCHA validation. Please make sure you have JavaScript & Cookies enabled on your browser.",'slider_captcha') );
			return $errors;
		}
	...
	public function validate_register_slider_buddypress($result) {
		...
		/* If someone tryies to hack */
		if ( !$this->valid_request() ) {
        	wp_die(__("ERROR: Something went wrong with the CAPTCHA validation. Please make sure you have JavaScript & Cookies enabled on your browser.",'slider_captcha'));
		}
	...
	public function validate_login_slider($user) {
		...
		if(isset($_REQUEST['wp-submit'])) {			
			if( !$this->valid_request() ) {
				...
				$error = new WP_Error();
				$error->add( 'slider_captcha_missing', __("ERROR: Something went wrong with the CAPTCHA validation. Please make sure you have JavaScript & Cookies enabled on your browser.",'slider_captcha') );
				return $error;
			} else {
				return $user;
			}
